...
if(isset($_POST['orden'])){
  $orden = json_decode(stripcslashes($_POST['orden']), TRUE);
  $x=1;
  foreach($orden as $value){
  $id = $value['id'];
    $sql = "UPDATE faq SET orden='$x' WHERE Id='$id'";
      ...
      <div class="hpanel">
        <div class="panel-body">
          <div class="dd" id="nestable">
            <ol class="dd-list">
              <?php
              $consulta = "SELECT * FROM faq ORDER BY orden";
              $resultado = mysql_query($consulta,$conexion);
              while($rArray = mysql_fetch_array($resultado)) {
                echo '<li class="dd-item" data-id="'.$rArray['Id'].'">
                <div class="dd-handle">'.$rArray['orden'].' - '.$rArray['pregunta'].'</div>
                </li>
              ';
              }
              ?>
          ...
          <form action="./?seccion=faq_order&nc=<?php echo $rand; ?>" method="post">
            <input type="hidden" id="orden" name="orden">
            <div class="pull-right">
              <a href="./?seccion=faq&id=<?php echo $catid; ?>&nc=<?php echo $rand; ?>" class="btn btn-warning">Volver</a>
